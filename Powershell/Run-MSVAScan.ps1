#remove older vulnerablity files older than 30 days
 if ((Get-ChildItem -Path "C:\Temp\SQL_Vulnerability_Scan").Count -gt 0)
 {
   
    Get-ChildItem -Path "C:\Temp\SQL_Vulnerability_Scan" -Recurse -File | Where LastWriteTime -lt  (Get-Date).AddMinutes(-10)  | Remove-Item -Verbose
}
else {
    Write-Host "There is not files to delte"

}


#Check if SQL is named or default
 if (get-itemproperty 'HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server' | Where-Object {$_.InstalledInstances -contains 'MSSQLSERVER'}) {
    $instance= $env:computername
}
else {
    $instance= $env:computername + "\" + (get-itemproperty 'HKLM:\SOFTWARE\Microsoft\Microsoft SQL Server').InstalledInstances
...
[string]$BaseFolder = (Get-Item -Path "C:\Temp" -Verbose).FullName
...
$output_path = "$BaseFolder\SQL_Vulnerability_Scan\"
...
$RulesDatabase = Import-Clixml -Path C:\Temp\RulesDatabase.xml
