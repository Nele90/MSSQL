...
/****** Object:  Job [DBA: Check Locked Users]    Script Date: 3/8/2023 12:46:53 PM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Database Maintenance]]    Script Date: 3/8/2023 12:46:53 PM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'DBA: Check Locked Users', 
		@enabled=1, 
		...
		@owner_login_name=N'ownername', @job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Check locked users]    Script Date: 3/8/2023 12:46:53 PM ******/
EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Check locked users', 
		@step_id=1, 
...
SELECT name ,is_disabled ,CAST(LoginProperty(name, ''IsExpired'') AS INT) is_expired 
...
IF EXISTS (SELECT 1 FROM #LockedUser) 
 BEGIN 
--Send email 
DECLARE @xml NVARCHAR(MAX)
DECLARE @body NVARCHAR(MAX)

SET @xml = CAST (( SELECT UserName AS ''td'','''', is_expired AS ''td'','''', is_disabled AS ''td'','''', is_locked AS ''td'',''''					
From #LockedUser

FOR XML PATH(''tr''), Elements ) AS NVARCHAR(MAX))


SET @body =''<html><body><H3>Locked User</H3>
<table border = 1> 
<tr>
<th> UserName </th> <th> is_expired </th> <th> is_disabled </th> <th>  is_locked </th>''    
...
EXEC @ReturnCode = msdb.dbo.sp_add_jobschedule @job_id=@jobId, @name=N'Every hour', 
		...
		@schedule_uid=N'6304daff-84d4-4fda-8543-c9339005b7a3'
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
